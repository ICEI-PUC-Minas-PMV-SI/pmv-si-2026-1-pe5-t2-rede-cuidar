...
com matriz em Belo Horizonte e unidades remotas (Secretaria e três UPAs).
O cenário exige alta disponibilidade e segmentação, dado o tráfego de
dados sensíveis e a necessidade de comunicação ininterrupta para
...
![Servidor Ubuntu On-Premise com IP estático configurado
(192.168.18.75/24)](./print_ip_estatico.png)

### Servidor Ubuntu (VirtualBox) — Serviços Ativos

![Serviços instalados e ativos no servidor Ubuntu
On-Premise](./print_servicos.png)

### Instância AWS EC2 — Nuvem

![Instância EC2 "Rede Cuidar" em execução no painel da
AWS](./print_instancia.png)

### Windows Server — Servidor de Aplicação

![Windows Server 2016 Datacenter em execução na instância
AWS](./windows_server.png)

### VPC — Rede Privada Virtual (AWS)

![VPC rede-cuidar-vpc configurada na AWS com CIDR
10.0.0.0/16](./print_vpc.png)

### Active Directory — Estrutura do Domínio

![Central Administrativa do Active Directory — domínio
minasgerais.net](./ad_print.png)

### Active Directory — Unidades Organizacionais (OUs)

![OUs criadas no AD: minas com sub-OUs belohorizonte, betim e
contagem](./subredes_print.png)

### Política de Grupo (GPO)

![GPO pgbh vinculada à OU usuarios em
belohorizonte](./gpo_criada.png)
...
![Mapa de monitoramento integrado da infraestrutura local e nuvem no
Zabbix](./print_mapa.png)
...
![Dashboard geral do Zabbix exibindo a saúde integrada do ambiente
hospitalar](./dashboard_print.png)
...
![Listagem de hosts ativos e com status operacional
estabilizado](./hosts_verdes.png)
...
![Métricas de uso de processador (CPU) no servidor Ubuntu
local](./cpu_utilization_ubuntu.png)
...
![Análise de alocação e consumo de memória RAM do servidor
local](./memory_utilization_ubuntu.png)
...
![Estatísticas de leitura e escrita em disco do servidor de banco de
dados](./disco_readwirte_ubuntu.png)
...
![Volume e taxa de transferência de pacotes na interface de rede
local](./network_traffic_ubuntu.png)

## Métricas de Performance — Servidor em Nuvem (srv-aws-cuidar)

A instância EC2 alocada na região Norte da Virgínia (`us-east-1`), que
atua como controlador de domínio Active Directory e hospedagem do
back-end hospitalar, teve suas métricas consolidadas.
...
![Métricas de processamento da instância EC2 Windows Server na
AWS](./cpu_utilization_aws.png)
...
![Gráfico de uso de memória física na instância de aplicação em
nuvem](./memory_utilization_aws.png)
...
dos limiares (*thresholds*) de checagem do Zabbix, a latência
geográfica devido à rota internacional foi devidamente mitigada, não
interferindo na qualidade da coleta contínua de dados do agente.

![Análise de pacotes enviados e recebidos através da interface WAN da
AWS](./rede_utilization_aws.png)
